...
//
//  expects otp, and token to verify account.
router.post(
  "/email-verification",
...
//
//  user signup done but user didn't verify email at that time, then verification starts from here
router.post(
  "/resend-otp",
...
router.post("/test-auth-token", async (req, res) => {
  ...
  try {
    ...
    if (role === allowed_roles.customer) {
